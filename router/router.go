...
// for _, activateProj := range projects {
// 	log.Printf("activating  projects %s\n", activateProj.Name)
// 	projId := activateProj.ID

// 	project, err := m.projBus.Get(ctx, projId)
// 	if err != nil {
// 		errChan <- err
// 		return
// 	}
...
// router := proj.GetRouter()

// m.router.Handl(activateProj.Name, router)
// server := http.Server{Addr: fmt.Sprintf(":%s", activateProj.Port), Handler: router}
// go func() {
...
// func (p *Project) GetRouter() *mux.Router {
// 	muxRouter := mux.NewRouter()

// 	projectRouter := p.GetRoutes()
// 	// sets  project name for mux router name
// 	for _, projectReq := range projectRouter {
// 		// build one handlerFunc  per  request
// 		path := projectReq.path
...
// 		muxRouter.Name(p.Name).Path(path).HandlerFunc(handler)
// 	}
